...
## What We Already Did
- On-demand rendering with a short warmup to avoid first-roll hitching (`Canvas frameloop="demand"` plus a few forced frames on mount and `invalidate()` on roll).
- Independent physics loop so Rapier runs while rendering stays idle (`Physics updateLoop="independent"`).
- Removed all shadow work (no `shadows` on `Canvas`, no shadow-casting lights/meshes, no `ContactShadows`).
